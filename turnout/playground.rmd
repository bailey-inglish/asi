  ...
  for (y in 2008 + (0:7 * 2)) {
    ...
    income_strength <-  cps_c %>%
      ...
      summarize(
        ...
        voted = round(sum(WTFINL * (VOTED == 2)), 1),
        pop_pct = 100 * round(count / sum(filter(cps_c, AGE >= 18 & CITIZEN < 5)$WTFINL), 3),
        voter_pct = 100 * round(voted / sum(cps_c$WTFINL * (cps_c$VOTED == 2)), 3),
