...
act_turn <- read_csv("raw_data/actual_turnout.csv") #1.2/0.3
...
raw_turn <- filter(cps, VOTED == 1 | VOTED == 2) %>%
...
# Write final outputs
write_csv(cps, "final_data/cps_revised_ipums_1984-2024.csv")
...
age_tab_tx3 <- cps %>%
  ...
  left_join(
    group_by(filter(cps, state_name == "Texas"), YEAR) %>%
      summarize(
